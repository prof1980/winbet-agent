...
def run_skill_scraper(bookmaker: str, league_id: str, output_file: str) -> dict:
    ...
    print(f"🎰 Skill scraping: {bookmaker} / {competition}")
    
    try:
        result = subprocess.run(cmd, capture_output=True, text=True, timeout=120)
        if result.returncode != 0:
            print(f"   ❌ Skill error: {result.stderr[:300]}")
            return {}
...
def import_to_db(data: dict, league_id: str):
    ...
    for event in data["events"]:
        ...
        # Inserisci quote per ogni mercato
        for market in event.get("markets", []):
            ...
            for sel in market.get("selections", []):
                ...
                c.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                """, (
                    match_id, bookmaker, mkt_type,
                    sel.get("name", ""), sel.get("label", ""),
                    odds, odds, datetime.now().isoformat()
                ))
                updated += 1
...
def main():
    ...
    if cfg["mode"] == "DEMO":
        print("🎲 DEMO mode: skill bridge non attivo (usa dati simulati)")
        print("   Per usare la skill reale: imposta mode='LIVE' e api_key_theoddsapi")
        return
    
    # LIVE mode: usa la skill per ogni bookmaker abilitato
    output_dir = "/mnt/c/Users/angel/WinBet/.tmp/skill_output"
    ...
    for bm in cfg.get("bookmakers", []):
        ...
        for league in cfg.get("leagues", []):
            ...
            league_id = league["id"]
            out_file = f"{output_dir}/{bm_id}_{league_id}.json"
            
            data = run_skill_scraper(bm_id, league_id, out_file)
            if data:
                count = import_to_db(data, league_id)
                total_updated += count
                print(f"   ✅ Importate {count} quote")
    
    print(f"\n📊 Totale quote importate dalla skill: {total_updated}")
