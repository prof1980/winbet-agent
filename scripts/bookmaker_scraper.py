...
class BaseAdapter(ABC):
    ...
    async def _navigate_with_delays(self, page: Any, url: str) -> None:
        ...
        for attempt in range(3):
            try:
                ...
                print(f"  ⚠ Navigation attempt {attempt+1} warning: {exc}")
                if attempt == 2:
                    return
    ...
    async def _interact_for_data(self, page: Any) -> None:
        """Scroll the page to trigger lazy-loaded content. Handles navigation gracefully."""
        for _ in range(2):
            try:
                await page.evaluate("window.scrollBy(0, window.innerHeight)")
                await _random_delay(0.8, 1.5)
            except Exception:
                pass  # Page may have navigated, ignore
        try:
